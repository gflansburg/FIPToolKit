...
{
	public abstract class FIPAltimeter : FIPAnalogGauge
	{
		public FIPAltimeter(FIPAltimeterProperties properties, FlightSimProviderBase flightSimProvider) : base(properties, flightSimProvider)
		{
            Properties.ControlType = GetType().FullName;
            AltimeterProperties.Name = string.Format("{0} Altimeter", flightSimProvider.Name);
							...
							if (AltimeterProperties.ShowKollsmanWindow)
							{
								using (StringFormat format = new StringFormat())
								{
									...
									using (Brush brush = new SolidBrush(AltimeterProperties.FontColor))
									{
										...
										grfx.DrawString(string.Format("{0:00.00}", FlightSimProvider.KohlsmanInchesMercury), AltimeterProperties.Font, brush, rectKollsman, format);
									}
								}
							}
