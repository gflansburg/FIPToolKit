...
namespace SpotifyAPI.Web
{
    internal class SpotifyWebClient : IClient
    {
        ...
        private const string TooManyRequestsJson = "{\"error\": { \"status\": 0, \"message\": \"SpotifyAPI.Web - Too Many Requests\" }}";
        private const string ForbiddenJson = "{\"error\": { \"status\": 0, \"message\": \"SpotifyAPI.Web - Forbidden\" }}";
        ...
        public Tuple<ResponseInfo, T> DownloadJson<T>(string url, Dictionary<string, string> headers = null)
        {
            ...
            {
                return new Tuple<ResponseInfo, T>(response.Item1, JsonConvert.DeserializeObject<T>(response.Item2.Equals("Too many requests") ? TooManyRequestsJson : response.Item2.Equals("Forbidden") ? ForbiddenJson : UnknownErrorJson, JsonSettings));
            }
        }
