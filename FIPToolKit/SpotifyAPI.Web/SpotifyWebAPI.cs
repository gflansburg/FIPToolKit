...
namespace SpotifyAPI.Web
{
    ...
    public sealed class SpotifyWebAPI : IDisposable
    {
        ...
        /// <returns></returns>
        public ErrorResponse ResumePlayback(string deviceId = "", string contextUri = "", List<string> uris = null, int? offset = null, int positionMs = 0)
        {
            JObject ob = new JObject();
        ...
        /// Only available when context_uri corresponds to an album or playlist object, or when the uris parameter is used.</param>
        /// <returns></returns>
        public Task<ErrorResponse> ResumePlaybackAsync(string deviceId = "", string contextUri = "", List<string> uris = null, int? offset = null, int positionMs = 0)
        {
            JObject ob = new JObject();
            if (!string.IsNullOrEmpty(contextUri))
                ob.Add("context_uri", contextUri);
        ...
        /// Only available when context_uri corresponds to an album or playlist object, or when the uris parameter is used.</param>
        /// <returns></returns>
        public ErrorResponse ResumePlayback(string deviceId = "", string contextUri = "", List<string> uris = null, string offset = "", int positionMs = 0)
        {
            JObject ob = new JObject();
            if (!string.IsNullOrEmpty(contextUri))
                ob.Add("context_uri", contextUri);
        ...
        public T DownloadData<T>(string url) where T : BasicModel
        {
            ...
            do
            {
                ...
                if (response.Item1.Headers.AllKeys.Contains("retry-after") && typeof(T) == typeof(PlaybackContext))
                {
                    (response.Item2 as PlaybackContext).RetryAfter = Convert.ToInt32(response.Item1.Headers["retry-after"]);
                }
                response.Item2.AddResponseInfo(response.Item1);
                lastError = response.Item2.Error;
                triesLeft -= 1;
        ...
        public string Download(string url)
        {
            ...
            Tuple<ResponseInfo, string> response = null;
            do
            {
                if (response != null) { Thread.Sleep(RetryAfter); }
                response = WebClient.Download(url, headers);
                triesLeft -= 1;
            } while (UseAutoRetry && triesLeft > 0 && response.Item2 == null);
            return response.Item2;
        }
