                        ...
                        if (!String.IsNullOrEmpty(FlightShareId) && IsStarted && !Stop)
                        {
                            ...
                            if (FlightShareIsLoggedIn)
                            {
                                ...
                                IRestRequest restRequest = new RestRequest("InsertPilotLocationWithSelect/{clientID}/{latitude}/{longitude}/{altitude}/{speed}/{magHeading}/{trueHeading}/{engineType}", DataFormat.Json)
                                    .AddUrlSegment("clientID", FlightShareId)
                                    .AddUrlSegment("latitude", SimConnect.CurrentAircraft.Position.Lat)
                                    .AddUrlSegment("longitude", SimConnect.CurrentAircraft.Position.Lng)
                                    .AddUrlSegment("altitude", SimConnect.CurrentAircraft.Altitude)
                                    .AddUrlSegment("speed", SimConnect.CurrentAircraft.IsOnGround ? SimConnect.CurrentAircraft.GroundSpeed : SimConnect.CurrentAircraft.IndicatedSpeed)
                                    .AddUrlSegment("magHeading", SimConnect.CurrentAircraft.HeadingMagnetic)
                                    .AddUrlSegment("trueHeading", SimConnect.CurrentAircraft.HeadingTrue)
                                    .AddUrlSegment("engineType", SimConnect.CurrentAircraft.Model);
